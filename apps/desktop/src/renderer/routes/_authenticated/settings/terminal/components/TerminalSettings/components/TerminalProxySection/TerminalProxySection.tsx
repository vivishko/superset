...
export function TerminalProxySection() {
	...
	useEffect(() => {
		setMode(terminalProxySettings?.mode ?? "auto");
	...
	return (
		<div className="rounded-md border border-border/60 p-4 space-y-4">
			...
			<div className="rounded-md border border-border/60 p-3 space-y-2">
				...
				{detectedQuery.data?.hasProxy ? (
					<div className="rounded-md border border-border/70 bg-muted/30 p-2.5">
						<div className="text-xs font-mono space-y-1 opacity-70">
							<div>HTTP_PROXY={detectedQuery.data.httpProxy || "—"}</div>
							<div>HTTPS_PROXY={detectedQuery.data.httpsProxy || "—"}</div>
							<div>NO_PROXY={detectedQuery.data.noProxy || "—"}</div>
						</div>
					</div>
				) : (
					<div className="rounded-md border border-border/70 bg-muted/30 p-2.5">
						<div className="text-xs font-mono space-y-1 opacity-70">
							<div>HTTP_PROXY={detectedValueFallback}</div>
							<div>HTTPS_PROXY={detectedValueFallback}</div>
							<div>NO_PROXY={detectedValueFallback}</div>
						</div>
					</div>
				)}
			</div>
